...
def ajouter_document(titre: str, description: str, type_document: str, cycle: str,
    ...
    # Apercu visuel automatique des premieres pages (nouveau, ajoute
    # apres la mise en production initiale -- voir apercu_documents.py).
    # Import local (et non en tete de fichier) afin qu'une eventuelle
    # absence de la dependance pymupdf, ou toute erreur de generation,
    # ne puisse jamais empecher le depot du document lui-meme : c'est
    # une amelioration secondaire, jamais bloquante.
    try:
        import apercu_documents
        apercu_documents.generer_apercu(nouveau_document_id, chemin_fichier)
    except Exception:
        pass
...
def supprimer_definitivement(document_id: int, supprime_par: int) -> tuple[bool, str]:
    ...
    # Nettoie aussi les images d'apercu visuel associees, le cas
    # echeant (nouveau, ajoute apres la mise en production initiale --
    # voir apercu_documents.py). Import local et non bloquant : meme
    # principe que dans ajouter_document.
    try:
        import apercu_documents
        apercu_documents.supprimer_apercu(document_id)
    except Exception:
        pass
